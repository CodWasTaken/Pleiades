...
mod tests {
    ...
    fn renders_links_as_colored_markdown_labels() {
        let renderer = TerminalRenderer::new();
        let output = renderer.render_markdown("See [Claw](https://example.com/docs) now.");
        let plain = strip_ansi(&output);

        assert!(plain.contains("[Claw](https://example.com/docs)"));
        assert!(output.contains('\u{1b}'));
    }
    ...
    fn highlights_fenced_code_blocks() {
        let renderer = TerminalRenderer::new();
        let output = renderer.markdown_to_ansi("```rust\nfn hi() { println!(\"hi\"); }\n```");
        let plain = strip_ansi(&output);

        assert!(plain.contains("╭─ rust"));
        assert!(plain.contains("fn hi"));
        assert!(output.contains('\u{1b}'));
        assert!(output.contains("[48;5;236m"));
    }

    #[test]
    fn renders_ordered_and_nested_lists() {
        let renderer = TerminalRenderer::new();
        let output = renderer.render_markdown("1. first\n2. second\n   - nested\n   - child");
        let plain = strip_ansi(&output);
    ...
    fn spinner_advances_frames() {
        ...
        let mut out = Vec::new();
        spinner.tick("Working", &theme, &mut out).expect("tick");
        spinner.tick("Working", &theme, &mut out).expect("tick");
